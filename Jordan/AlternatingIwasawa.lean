...
theorem mulAut_smul_subgroupOf_eq {G : Type _} [Group G] {N H : Subgroup G} (f : MulAut G)
    (f' : MulAut N) (hff' : ∀ n : N, f n = f' n) : (f • H).subgroupOf N = f' • H.subgroupOf N :=
  by
  ...
  · rintro ⟨y, hy, hyx⟩
    use y
    · -- y ∈ N
      suffices y = f'.symm x by rw [this]; apply SetLike.coe_mem
...
def Subgroup.mulEquivOfMulEquiv {G G' : Type _} [Group G] [Group G'] (f : G ≃* G') {H : Subgroup G}
  ...
  · apply MonoidHom.codRestrict (MonoidHom.restrict f.to_monoid_hom H) H'
    rintro ⟨g, hg⟩
    simp only [MonoidHom.restrict_apply, Subgroup.coe_mk, MulEquiv.coe_toMonoidHom]
    rw [← h]; exact hg
  · apply MonoidHom.codRestrict (MonoidHom.restrict f.symm.to_monoid_hom H') H
    rintro ⟨g', hg'⟩
    rw [h _]
    simp only [MonoidHom.restrict_apply, Subgroup.coe_mk, MulEquiv.coe_toMonoidHom,
...
def iwConj (s : Finset α) (g : Equiv.Perm α) : Equiv.Perm s ≃* Equiv.Perm (g • s : Finset α) :=
  {
    Equiv.permCongr
      (@Equiv.subtypeEquiv α α (fun a => a ∈ s) (fun a => a ∈ g • s) (g : α ≃ α)
        (fun a => by rw [← Finset.smul_mem_smul_finset_iff g, Equiv.Perm.smul_def] :
          ∀ a : α, a ∈ s ↔ g a ∈ g • s)) with
    map_mul' := fun h k =>
      Equiv.ext fun x => by
        simp only [Equiv.toFun_as_coe, Equiv.permCongr_apply, Equiv.subtypeEquiv_symm,
          Equiv.subtypeEquiv_apply, Equiv.Perm.coe_mul, Function.comp_apply, Subtype.coe_mk,
          Equiv.symm_apply_apply, Finset.mk_coe] }
#align equiv.perm.Iw_conj Equiv.Perm.iwConj

def iwConj' {s t : Finset α} {g : Equiv.Perm α} (htgs : t = g • s) : Equiv.Perm s ≃* Equiv.Perm t :=
  {
    Equiv.permCongr
...
theorem iwConj_symm_sign (s : Finset α) (g : Equiv.Perm α) (k : Equiv.Perm (g • s : Finset α)) :
  ...
  suffices ∀ k, (Iw_conj s g).symm k = (Equiv.permCongr e.symm) k
    by
    rw [this]
...
theorem Iw_is_conj_alt (s : Finset α) (g : alternatingGroup α) :
  ...
  · simp only [← Subgroup.map_map]
    ...
    · intro hk
      ...
      simp only [Iw_conj_symm_sign, MulEquiv.apply_symm_apply]
