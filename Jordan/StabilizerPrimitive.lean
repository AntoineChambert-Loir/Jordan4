...
theorem ofSubtype_mem_stabilizer (s : Set α) (g : Equiv.Perm s) :
    g.ofSubtype ∈ stabilizer (Equiv.Perm α) s :=
  by
...
theorem stabilizer_isPreprimitive (s : Set α) : IsPreprimitive (stabilizer (Equiv.Perm α) s) s :=
  ...
  let f : s →ₑ[φ] s :=
    { toFun := id
      map_smul' := fun g x => by simp only [id.def, Equiv.Perm.smul_def, to_perm_apply] }
...
theorem Stabilizer.isPreprimitive (s : Set α) (hs : (sᶜ : Set α).Nontrivial) :
  ...
  have hks : Equiv.Perm.ofSubtype k • s = s :=
    by
  ...
  have hφ : Function.Surjective φ :=
    by
    have hminus_one_ne_one : (-1 : Units ℤ) ≠ 1 := by intro h;
      rw [← Units.eq_iff, Units.val_one, Units.coe_neg_one] at h ; norm_num at h 
    intro g
    let g' :=
      ite (Equiv.Perm.sign g = 1) (Equiv.Perm.ofSubtype g)
        (Equiv.Perm.ofSubtype g * Equiv.Perm.ofSubtype k)
    use g'
...
theorem Stabilizer.is_preprimitive' (s : Set α) (hsc : sᶜ.Nontrivial) (G : Subgroup (Equiv.Perm α))
    (hG : stabilizer (Equiv.Perm α) s ⊓ alternatingGroup α ≤ G) :
    IsPreprimitive (stabilizer G s) s :=
  by
  let φ : stabilizer (alternatingGroup α) s → stabilizer G s := fun g =>
    by
    use (g : alternatingGroup α); apply hG; rw [Subgroup.mem_inf]
    constructor
  ...
  exact hsc
#align alternating_group.stabilizer.is_preprimitive' alternatingGroup.Stabilizer.is_preprimitive'
