...
theorem isPretransitive_ofFixingSubgroup_inter
  ...
  · -- x ∉ s
    ...
    · simp only [← SetLike.coe_eq_coe] at hkax ⊢
      exact hkax
  · -- x ∉ g • s
    suffices hg'x : g⁻¹ • x ∈ SubMulAction.ofFixingSubgroup G s
    suffices hg'a : g⁻¹ • a ∈ SubMulAction.ofFixingSubgroup G s
